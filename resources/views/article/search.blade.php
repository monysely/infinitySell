...
<x-slot name="title">InfinitySell - {{ ucfirst($word) }}</x-slot>
    
    <x-second-hero/>
    
    <div class="container mt-5">
        <div class="row">
            <p class="text-center py-3 fs-3">
                {{ ucfirst($word) }}
            </p>
            ...
            <div class="col-12 col-md-6 col-lg-4 col-xl-3 animate__animated animate__flipInX">
                <a href="{{ route('article.detail', ['title'=> Str::slug($article->title), 'id'=>$article->id]) }}" class="text-decoration-none text-dark">
                    <div class="card mb-5 shadow position-relative bianco-sporco">
                        ...
                        <div class="position-absolute bagde bg-2 text-white ms-2 mt-2 px-1 rounded-3 top-0 start-0 small">
                            <a href="{{route('article.category',['title'=>$article->category->ita, 'category' => $article->category_id])}}" class="text-decoration-none text-white"> @if (App::getLocale() == 'it') {{ucfirst($article->category->ita)}}  @elseif(App::getLocale() == 'en') {{ucfirst($article->category->en)}} @else {{ucfirst($article->category->ro)}} @endif </a>
                        </div>
                    </div>
                </a>
            </div>
